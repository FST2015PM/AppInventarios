...
{
    [Activity(Label = "CableadoPOCActivity")]
    public class CableadoPOCActivity : Activity
    {
        GoogleMap map;
        LatLng latLngSource;
        ...
        async void FnProcessOnMap()
        {
            ...
            if (latLngSource != null && latLngDestination != null)
                FnDrawPath("Calderitas,Quintana Roo,Mexico", "Chetumal,Quintana Roo,Mexico");
        }
        async void FnDrawPath(string strSource, string strDestination)
        {
            string strFullDirectionURL = string.Format("https://maps.googleapis.com/maps/api/directions/json?origin={0}&destination={1}&key=", strSource, strDestination);
            string strJSONDirectionResponse = await FnHttpRequest(strFullDirectionURL);
            if (strJSONDirectionResponse != "Exception")
            {
                RunOnUiThread(() =>
                {
                    if (map != null)
                    {
                        map.Clear();
                        MarkOnMap("Punto A", latLngSource, Resource.Drawable.PinIcon);
                        MarkOnMap("Punto B", latLngDestination, Resource.Drawable.PinIcon);
                    }
                });
            ...
            {
                RunOnUiThread(() =>
                    Toast.MakeText(this, "No es posible conectar", ToastLength.Short).Show());
            }
        ...
        async Task<bool> FnLocationToLatLng()
        {
            try
            {
                ...
                var sourceAddress = await geo.GetFromLocationNameAsync("Calderitas,Quintana Roo,Mexico", 1);
                sourceAddress.ToList().ForEach((addr) =>
                {
                    latLngSource = new LatLng(addr.Latitude, addr.Longitude);
                });

                var destAddress = await geo.GetFromLocationNameAsync("Chetumal,Quintana Roo,Mexico", 1);
